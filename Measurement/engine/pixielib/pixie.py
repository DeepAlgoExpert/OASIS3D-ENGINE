...
class PIXIE(object):
    ...
    def convert_pose(self, param_dict, param_type):
        ...
        if param_type == 'head':
            ...
            param_dict['partbody_pose'] = self.smplx.body_pose.unsqueeze(0).expand(batch_size, -1, -1, -1)[:,:self.param_list_dict['body_list']['partbody_pose']]
            ...
            dtype = param_dict['right_hand_pose'].dtype; device=param_dict['right_hand_pose'].device
    ...
    def decode(self, param_dict, param_type):
        ...
        if param_type == 'head' or param_type == 'body':
            param_dict['body_pose'] = self.smplx.pose_abs2rel(param_dict['global_pose'], param_dict['body_pose'], abs_joint='head')
        if param_type == 'hand' or param_type == 'body':
            param_dict['body_pose'] = self.smplx.pose_abs2rel(param_dict['global_pose'], param_dict['body_pose'], abs_joint='left_wrist')
            param_dict['body_pose'] = self.smplx.pose_abs2rel(param_dict['global_pose'], param_dict['body_pose'], abs_joint='right_wrist')
        ...
        if 'tex' in param_dict.keys() and self.cfg.model.use_tex:
            ...
            prediction['albedo'] = torch.zeros([verts.shape[0], 3, self.cfg.model.uv_size, self.cfg.model.uv_size], device=self.device, dtype=torch.float32)
